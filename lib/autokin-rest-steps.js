...
Then('I expect that {string} has value of {string}', function (path, value) {
    expect(value).to.eql(Builder.Response().Body().pathValue(path));
});

Then('I expect that {string} has value of {int}', function (path, value) {
    expect(value).to.eql(Builder.Response().Body().pathValue(path));
});

Then('response body should have JSON path {string}', function (path, callback) {
