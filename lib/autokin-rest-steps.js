...
Then(
    'I save data from path {string} to file {string} that matches',
    (jsonpath, filepath, matchers, callback) => {
        const matcherKeys = {};
        matchers.hashes().forEach((matcher) => {
            matcherKeys[Store.sanitize(matcher.name)] = Store.sanitize(matcher.value);
        });

        const matches = Builder.Response()
            .Body()
            .filterObjectKeys(Store.sanitize(jsonpath), matcherKeys);
        if (matches.length > 0) {
            const destination = Store.sanitize(filepath);
            const folder = path.dirname(destination);
            if (!fs.existsSync(folder)) {
