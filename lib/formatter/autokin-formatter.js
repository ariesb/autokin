...
class AutokinFormatter extends JsonFormatter {
    constructor(options) {
        ...
        options.eventBroadcaster.on('test-step-started', ({ testCase, index }) => {
            ...
            this.spinner.text = `\t ${colors.yellow(gherkinKeyword)}${Store.sanitize(pickleStep.text)}`;
            this.spinner.start();
        });
        ...
        options.eventBroadcaster.on('test-step-finished', ({ testCase, index, result }) => {
            ...
            let message = `${this.resolveStatus(result.status)} - ${colors.yellow(gherkinKeyword)}${Store.sanitize(pickleStep.text)}`;
            if(result.status == 'failed') {
                message += `\n\t - ${colors.red(result.exception.message)}`;
            }
            this.spinner['_' + result.status](`${message}${this.displayDuration(result)}`);
        });
