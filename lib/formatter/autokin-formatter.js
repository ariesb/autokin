        ...
        options.eventBroadcaster.on('test-case-finished', ({ sourceLocation, result }) => {
            ...
            if (result.status != 'passed') {
                testCase.steps.forEach((step, index) => {
                    if (['failed','undefined'].indexOf(step.result.status) > -1) {
                        const { gherkinKeyword, pickleStep } = options.eventDataCollector.getTestStepData(
                            {
                                testCase,
                                index
                            });
                        let message = step.result.status == 'undefined' ? 'Please correct invalid Gherkin Step.' : result.exception.message;
                        status += `\n\t\t${colors.yellow(gherkinKeyword)}${pickleStep.text} - ${colors.red(message)}`;
                    }
                });
            }
            this.logFn(` - ${status} \n`);
